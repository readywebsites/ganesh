...
class GalleryAdmin(admin.ModelAdmin):
    list_display = ('image_preview', 'title', 'category', 'status', 'is_featured', 'created_at', 'updated_at')
...
class DonationAdmin(admin.ModelAdmin):
    list_display = ('receipt_preview', 'donor_name', 'email', 'phone', 'amount', 'currency', 'transaction_id', 'payment_method', 'status', 'created_at')
    list_filter = ('status', 'payment_method', 'created_at')
    search_fields = ('donor_name', 'email', 'phone', 'transaction_id')
    ordering = ('-created_at',)
    list_editable = ('status',)
    readonly_fields = ('id', 'receipt_preview_large', 'created_at', 'updated_at')
    ...
    def receipt_preview(self, obj):
        ...
        return format_html('<span style="color: #999;">No Receipt</span>')
...
class MembershipAdmin(admin.ModelAdmin):
    list_display = ('photo_preview', 'full_name', 'email', 'phone', 'membership_tier', 'status', 'created_at')
...
class ContactAdmin(admin.ModelAdmin):
    list_display = ('attachment_preview', 'name', 'email', 'phone', 'subject', 'status', 'created_at')
    list_filter = ('status', 'created_at')
    search_fields = ('name', 'email', 'phone', 'subject', 'message')
    ordering = ('-created_at',)
    list_editable = ('status',)
    readonly_fields = ('id', 'attachment_preview_large', 'created_at', 'updated_at')
